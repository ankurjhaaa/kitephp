...
<body class="bg-[#0f111a] text-gray-300 font-sans p-6 md:p-12 min-h-screen selection:bg-blue-500/30 selection:text-blue-200">
    <div class="max-w-5xl mx-auto">
        ...
        <div class="bg-green-900/30 border-l-4 border-green-500 text-green-300 px-4 py-3 rounded mb-8 shadow-sm flex items-center gap-3">
        ...
        <div class="bg-red-900/30 border-l-4 border-red-500 text-red-300 px-4 py-3 rounded mb-8 shadow-sm flex items-center gap-3">
        ...
        <div class="bg-[#161b22] rounded-xl border border-gray-800 shadow-xl overflow-hidden">
            <div class="p-5 border-b border-gray-800 bg-[#1c2128] flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                    <form action="{{ route('users.index') }}" method="GET" kite:submit="search" class="flex gap-2 w-full sm:w-auto">
                        <input type="text" name="search" placeholder="Search by name..." value="{{ $search ?? '' }}" class="w-full sm:w-64 bg-[#0d1117] border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:outline-none focus:border-blue-500 transition-colors" autocomplete="off">
                        <button type="submit" class="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg text-sm font-medium transition-colors border border-gray-600">Search</button>
                        @if(!empty($search))
                            <a href="{{ route('users.index') }}" kite:navigate="clearSearch" class="px-3 py-2 bg-red-900/50 hover:bg-red-800/50 text-red-400 rounded-lg text-sm font-medium transition-colors border border-red-800/50 flex items-center" title="Clear Search">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </a>
                        @endif
                    </form>
                    
                    <button onclick="document.getElementById('userModal').classList.remove('hidden')" class="w-full sm:w-auto px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded shadow-lg shadow-blue-900/20 transition-colors flex justify-center items-center gap-2 whitespace-nowrap">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-[#0d1117] text-gray-500 text-xs uppercase tracking-wider border-b border-gray-800">
        ...
        <div class="bg-[#161b22] w-full max-w-md rounded-xl border border-gray-800 shadow-2xl relative transition-all @if(errors()) ring-1 ring-red-500/50 @endif">
            <div class="p-6">
                <div class="flex justify-between items-center mb-5">
                    ...
                    @if($editUser)
                        <a href="{{ route('users.index') }}" kite:navigate="close" class="text-gray-400 hover:text-white transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </a>
                    @else
                        <button onclick="document.getElementById('userModal').classList.add('hidden')" class="text-gray-400 hover:text-white transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                        </button>
                    @endif
                </div>
                ...
                @endif
                    ...
                    <div class="flex gap-3">
                        <button type="submit" class="bg-blue-600 hover:bg-blue-500 text-white font-semibold py-2.5 px-4 rounded-lg transition-colors w-full shadow-lg shadow-blue-900/20">
                            @if($editUser) Update User @else Save User @endif
                        </button>
                        @if($editUser)
                            <a href="{{ route('users.index') }}" kite:navigate="users" class="bg-gray-800 hover:bg-gray-700 text-gray-300 font-semibold py-2.5 px-4 rounded-lg text-center transition-colors border border-gray-700 w-full">Cancel</a>
                        @else
                            <button type="button" onclick="document.getElementById('userModal').classList.add('hidden')" class="bg-gray-800 hover:bg-gray-700 text-gray-300 font-semibold py-2.5 px-4 rounded-lg text-center transition-colors border border-gray-700 w-full">Cancel</button>
                        @endif
                    </div>
                </form>
            </div>
